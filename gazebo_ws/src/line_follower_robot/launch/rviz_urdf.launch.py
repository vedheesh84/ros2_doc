...
with joint_state_publisher GUI and robot_state_publisher.
"""
...
def generate_launch_description():
    ...
    # Path to URDF file
    urdf_file = os.path.join(pkg_share_dir, 'urdf', 'line_follower_robot.urdf')
    ...
    robot_state_publisher_node = Node(
        ...
        output='screen',
        parameters=[{'robot_description': open(robot_description_content).read()}]
    )
    ...
    rviz_node = Node(
        ...
        name='rviz2',
        output='screen',
        arguments=['-d', os.path.join(pkg_share_dir, 'config', 'rviz_line_follower.rviz')]
    )
